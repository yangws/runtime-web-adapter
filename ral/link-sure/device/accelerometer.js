...
if (wuji.offAccelerometerChange) {
    ...
    jsb.onAccelerometerChange = function (listener) {
        if (typeof listener === "function") {
            ...
            if (length === 0) {
                wuji.onAccelerometerChange(_onAccelerometerChange);
            }
        }
    };
    jsb.offAccelerometerChange = function (listener) {
        ...
        for (let index = 0; index < length; ++index) {
            if (listener === _listeners[index]) {
                ...
                if (length === 1) {
                    wuji.offAccelerometerChange(_onAccelerometerChange);
                }
                return;
            }
        }
    ...
    jsb.startAccelerometer = wuji.startAccelerometer.bind(wuji);
